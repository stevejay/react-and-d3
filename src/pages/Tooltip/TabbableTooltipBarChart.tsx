...
export type TabbableTooltipBarChartProps<CategoryT extends AxisDomain> = {
  ...
  renderTooltipContent: (datum: CategoryValueDatum<CategoryT, number>) => string;
...
function TabbableTooltipBarChartCore<CategoryT extends AxisDomain>({
...
}: TabbableTooltipBarChartProps<CategoryT>): ReactElement<any, any> | null {
  ...
  const [tooltipState, tooltipEventHandlers, svgRef, tippyProps] =
    useTabbableTooltip<CategoryValueDatum<CategoryT, number>>();
  return (
    <>
      <SvgChartRoot
        ref={svgRef}
      ...
      <Tippy
        {...tippyProps}
        render={(attrs) => (
          <Tooltip {...attrs}>{tooltipState.datum && renderTooltipContent(tooltipState.datum)}</Tooltip>
        )}
      />
    </>
  );
