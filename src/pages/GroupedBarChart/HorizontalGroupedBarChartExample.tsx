...
const dataSets = [
  {
    ...
    data: [
      { category: 'A', values: { one: -2, two: 0, three: 0 } },
  ...
  },
  {
    ...
    data: [
      ...
      { category: 'C', values: { one: 81.32, two: 50, three: 60 } },
      { category: 'D', values: { one: -102.974, two: 10, three: 20 } },
      { category: 'E', values: { one: 87.247, two: 10, three: 40 } }
    ]
  },
  {
    seriesKeys: ['two'],
...
export function HorizontalGroupedBarChartExample() {
  const [data, nextDataSet] = useSeriesDataSets(dataSets);
  return (
    <div className="my-8">
      <div className="relative w-full h-[512px]">
        <InView>
          <HorizontalGroupedBarChart data={data.data} dataKeys={data.seriesKeys} />
        </InView>
      </div>
