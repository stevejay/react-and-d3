...
function Axis() {
  return (
    <main className="w-full max-w-3xl p-4 mx-auto md:p-8">
      ...
      <Paragraph>
        The D3 axis component is a stalwart of D3 data visualisations. If you want to use React instead of D3
        to render your charts then you are going to need a suitable replacement. This page demonstrates my
        port of the D3 axis component. As with that component, ticks that are entering start in the position
        they would have been in according to the previous scale. And ticks that are exiting move to the
        position they would be in according to the new scale. You can see some examples below of D3 axes
        alongside their React equivalents.
      </Paragraph>
      <ChartTitle>Example 1: Linear axis</ChartTitle>
      <LinearAxisExamples transitionSeconds={0.5} springConfig={fastSpringConfig} />
      <ChartTitle>Example 2: Time axis (clamped)</ChartTitle>
      ...
      <Paragraph>
        ...
        interrupted. An interruption occurs when the scale is updated while the axis is still transitioning.
        The problem is with ticks that are exiting and still need to continue to exit. They continue to
        animate to a position according to the previous scale rather than a position according to the new
        scale.
      </Paragraph>
      <Paragraph>
        This is only really obvious if the animations are slowed down, as in the following example. Click on
        the update button while the axis is mid-animation to see the difference.
      </Paragraph>
